...
variant_mutation_rate = function(gene, cadd_variants, cesa, cadd_threshold = 20){
  gene_x = gene
  #' 
  #' ASSUMPTION: Only take AMINO ACID CHANGING mutations
  #' 
  # unlist because some codons have multiple variants
  variants = unlist(cesa$variants[gene == gene_x & variant_type == 'aac',constituent_snvs])
  ...
  all_trinuc_mut_props = compute_trinucleotide_mut_proportions(gene_x, cesa)
  variants$trinuc_mut_prop = as.vector(all_trinuc_mut_props[variants$trinuc_mut])
