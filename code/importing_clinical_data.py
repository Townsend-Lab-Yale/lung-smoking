...
for i in files:
    df1 = pd.read_csv(os.path.join("data/clinical_data/",i, "data_clinical_patient.txt"), sep="\t", comment="#")
    df2 = pd.read_csv(os.path.join("data/clinical_data/",i, "data_clinical_sample.txt"), sep="\t", comment="#")
...
msk2017_df = msk2017_df[["SAMPLE_ID", "SMOKING_HISTORY", "STAGE_AT_DIAGNOSIS", "VITAL_STATUS","Treatment","SAMPLE_TYPE"]]
#using sample_id instead of patient_id because that is what matches with patient_id in the maf file
#msk2017 only has vital status rather than PFS or OS.
msk2017_df.columns = ["Sample ID","Smoker","Stage","Vital Status","Treatment","Metastatic"]
...
msk2018_df = msk2018_df[['SAMPLE_ID','SMOKER','Stage','PFS_MONTHS','Treatment','Metastatic','is_LUAD']]
msk2018_df.columns = ["Sample ID","Smoker","Stage","Progression Free Survival (months)","Treatment","Metastatic","is_LUAD"]
...
#not sure regression free survival = progression free survival
